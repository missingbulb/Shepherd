#!/usr/bin/env node
// The claudinite-fleet-sheepdog pack's fleet PACK-SEED sweep — the enforcer making sure every member
...
// this repo's claudinite-fleet-sheepdog entry (fleet-config.mjs). That is the whole point: the enforcer
...
export async function main() {
  ...
  if (homeCfg === null) throw new Error(`the claudinite-fleet-sheepdog repo ${home} has no readable ${DECLARATION}`);
  ...
  if (!packSeeds.length) {
    ...
    const none = `# Fleet pack seeds — ${owner}\n\nNo \`packSeeds\` on this repo's claudinite-fleet-sheepdog entry: this fleet asks its members to declare nothing in particular.`;
    console.log(none);
...
if (isMain) {
  main().catch((e) => { console.error(`fleet-pack-seeds sweep failed: ${e.message}`); process.exitCode = 1; });
}
