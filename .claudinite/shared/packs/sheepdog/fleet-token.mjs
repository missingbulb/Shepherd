...
export function fleetTokenHandoverStep() {
  return {
    ...
    breaks: 'every sheepdog sweep fails — and a token short one permission fails only on the sweep that needs it, '
      + 'which can be a week later, on the one sweep that writes or dispatches',
    done: `the secret exists and each sheepdog task's next run is green`,
  };
}
