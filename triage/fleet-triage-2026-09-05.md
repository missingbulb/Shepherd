# Fleet triage — 2026-09-05

366 open issues, 16 repos, **133 → 146 queue-managed / 220 plain**. Second run in the series;
the 2026-09-02 run is the baseline it is read against.

**This report runs on two-day-old data, and that is the headline finding, not a caveat.**

## The triage's own data source is dead, by the deadlock this series predicted

The snapshot task is `frequency: daily`. Its last output landed 2026-09-03. There is none for
09-04 or 09-05, and none will arrive.

Shepherd#449, the snapshot's own work item, parked `failure` on 2026-09-04 08:50 and has not moved
since. `failure` is the one blocking park kind — it holds the lane — so **no further occurrence of
the task has been filed**, confirmed live: Shepherd's open queue today carries exactly one
snapshot item, the parked one. The fleet-drift sweep ran normally at 08:20 today, so this is not a
dead scheduler; it is one frozen lane.

The 09-02 report argued this deadlock structurally: a `failure` park is supersedable only by a
later clean run of the same task, which the park itself prevents from ever being filed. Three days
later it has closed on the triage's own collection step.

### Root cause, reproduced locally

```
ERR_MODULE_NOT_FOUND
url: '…/local/packs/shepherd/tasks/fleet-issues-snapshot/task.mjs'
```

Canon's `taskDeclarationsToJson` migration converted the task declaration from `task.mjs` to
`task.json` and deleted the module — but did not rewrite the two files that import it:

- `worker.mjs:27` — `import task from './task.mjs'` (this is what crashes the run)
- `task.test.mjs:3` — the same import

Running the task's own test in this checkout fails identically, so the break is live and not a
runner artifact. The migration landed 2026-09-03 13:51; the first failed run was 2026-09-04 08:50.

**This is a canon migration defect, and its blast radius is every member whose local pack had a
`task.mjs` that its own worker or test imported.** A migration that rewrites a declaration must
rewrite that declaration's importers, or it converts a working task into a blocking park. Shepherd
is simply the member where the damage is visible, because the frozen task is the one this skill
reads.

## Nothing drained. Not one item.

The 09-02 and 09-03 snapshots were compared item by item, keyed on `(repo, number)`:
...
All 129 are the *same items*. In a window with an hourly scheduler and a janitor sweeping the
whole fleet, not one park cleared, was superseded, or changed kind. The 09-02 report predicted
the parked population can only accumulate; this is that prediction measured rather than argued.

The window is ~21 hours, not three days — the second snapshot is 09-03, because of the freeze
above. **The 09-04 and 09-05 movement is unmeasurable**, and will stay so until the lane is fixed.

## Lane cost: 244 missed runs, up from 179

Same method as the 09-02 run — each blocking park's age from `created_at`, against its task's
declared `frequency` at HEAD. **+65 missed runs in three days**, from 36 blocking parks that are
the identical 36 items.

Worst: ClaudiniteCanary#126 (17), VascularColoring#156 (16), Shepherd#117 and Claudinite#1146 (15
each). `growth-extract` and `tidy-issues` are daily and still dominate the cost.

## Two corrections to this run's own first reading

Both came from sampling, and both would have been wrong in the report otherwise.

**The `no-status` cohort does not exist.** The classifier reported 4 queue items carrying no status
label at all — rule D's stateless shape, a torn label swap. They are Claudinite#1640–1643. Read
live, all four carry `task:status:blocked` normally. The snapshot was generated 08:56; the four
were created 08:51 and labelled 10:04. **The snapshot caught a five-minute window between creation
and labelling and froze it**, and the freeze above then preserved that artifact for two days. There
is no torn-swap cohort in this fleet; there is a stale-snapshot artifact that looks exactly like one.

**The `blocked` growth is healthy machinery, not backlog.** `blocked` rose from 1 to 9, which reads
as stalling. Sampling #1640 and #1643 shows both carry `Blocked-by: #1637` and
`Not-before: 2026-09-10T09:00:00Z` — sub-issues of the #1638 legacy-retirement chain, deliberately
asleep until 09-10. On the skill's wait-vs-sleep axis these are *sleeping*: the mechanism working
exactly as designed. Counting them as stuck would have overstated the backlog by six.

## What actually grew

+25 issues opened, 8 closed, net +17. The queue's +13 is almost entirely Claudinite (91 → 103),
and it is that same migration chain — five blocked `Verify in production` / `Retrospective`
sub-issues plus the four `Retire the …` items. Every one of them is ad-hoc, scheduled, and asleep.

**The parked backlog did not grow. It did not shrink either. It is exactly where it was.**

## Coverage gap: a member the snapshot has never seen

Shepherd#458, filed today, reports Claudinite drift on `missingbulb/norfinder`. `norfinder` is not
among the snapshot's 16 repos. It is plausibly a member adopted after 09-03 — but the mechanism
that would pick it up is the frozen lane, so **the fleet's own inventory cannot self-correct while
the snapshot is down.** Every count in this report is over 16 repos and may be over 17.

## Still true from 09-02, unchanged

- 53% of parks (`decision`/`approval`/bare) are neither blocking nor supersedable — nothing drains
  them. Upstream as missingbulb/Claudinite#1538.
- ~32 `decision` parks are mislabelled dead-agent reclaims from the pre-09-01 engine.
- ClaudiniteWebsite#204 (since 08-23) and EdFringeNow#401 (since 08-18) still report their
  schedulers dead. Both untouched in the interval.

## Recommendations

1. **Fix the two imports.** Two lines in this repo's local pack, and the snapshot lane runs again.
   Until then every future run of this skill reads frozen data — and reads it without knowing,
   which is worse than reading none.
2. **Clear Shepherd#449 after the fix** — it will not clear itself; that is the deadlock.
3. **Take the migration defect to canon.** The two-line fix here does nothing for the other members
   the migration touched the same way.
4. Everything the 09-02 run recommended still stands, none of it actioned.

---
# Fleet triage — every open issue, 2026-09-03

366 open issues across 16 in-scope repos under `missingbulb` (snapshot generated 2026-09-03T08:56:16.852Z).
`Q` = queue-managed. `Gen` = label generation (canon `task:status:*` vs retired `needs-human`/`task:needs-human-*`).


## missingbulb/Claudinite — 103 open (31 queue / 72 plain)

| # | Q | State | Gen | Updated | Title |
|---|---|-------|-----|---------|-------|
| [#1643](https://github.com/missingbulb/Claudinite/issues/1643) | Q | no-status | none | 2026-09-03 | Retire the remaining scattered legacy residues |
| [#1642](https://github.com/missingbulb/Claudinite/issues/1642) | Q | no-status | none | 2026-09-03 | Retire the task contract and queue vocabulary legacy tolerances |
| [#1641](https://github.com/missingbulb/Claudinite/issues/1641) | Q | no-status | none | 2026-09-03 | Retire the renamed and absorbed pack id tolerances |
| [#1640](https://github.com/missingbulb/Claudinite/issues/1640) | Q | no-status | none | 2026-09-03 | Retire the member declaration and stamp legacy shapes |
| [#1639](https://github.com/missingbulb/Claudinite/issues/1639) |  | unlabelled-backlog |  | 2026-09-03 | Make fleet legacy-tolerance clearance readable from this repo |
...
| [#1637](https://github.com/missingbulb/Claudinite/issues/1637) |  | unlabelled-backlog |  | 2026-09-03 | Legacy tolerance is scaffolding: guideline, site annotation, and member-facing advisories |
| [#1635](https://github.com/missingbulb/Claudinite/issues/1635) |  | unlabelled-backlog |  | 2026-09-02 | task.json: canon support, schema, converter and the member migration record |
| [#1633](https://github.com/missingbulb/Claudinite/issues/1633) |  | unlabelled-backlog |  | 2026-09-02 | Task declarations move from task.mjs to task.json |
| [#1632](https://github.com/missingbulb/Claudinite/issues/1632) |  | unlabelled-backlog |  | 2026-09-02 | Every pack's RULES.md opens with description that belongs in its README |
| [#1628](https://github.com/missingbulb/Claudinite/issues/1628) | Q | blocked | canon | 2026-09-02 | Verify in production: no member holds the cleaned engine beside a legacy task declaration |
...
| [#1623](https://github.com/missingbulb/Claudinite/issues/1623) | Q | blocked | canon | 2026-09-02 | Verify in production: the usage fold's new counters reach a real fold |
| [#1620](https://github.com/missingbulb/Claudinite/issues/1620) |  | unlabelled-backlog |  | 2026-09-02 | logs-prune: retention_days needs a default (10 days), not an unset that silently disables the prune |
| [#1610](https://github.com/missingbulb/Claudinite/issues/1610) | Q | blocked | canon | 2026-09-02 | Verify in production: the published fleet dashboard carries the corpus section |
...
## missingbulb/EdFringeNow — 24 open (7 queue / 17 plain)

| # | Q | State | Gen | Updated | Title |
|---|---|-------|-----|---------|-------|
| [#587](https://github.com/missingbulb/EdFringeNow/issues/587) |  | unlabelled-backlog |  | 2026-09-02 | Design mockups: festival trip planner (choose festival → priorities → proposed schedule → content chooser) |
...
## missingbulb/Shepherd — 24 open (14 queue / 10 plain)

| # | Q | State | Gen | Updated | Title |
|---|---|-------|-----|---------|-------|
| [#432](https://github.com/missingbulb/Shepherd/issues/432) | Q | running-executor | canon | 2026-09-03 | [claudinite-work] shepherd/fleet-issues-snapshot |
| [#431](https://github.com/missingbulb/Shepherd/issues/431) | Q | waiting-for-executor | canon | 2026-09-03 | [claudinite-work] tidy-repo/tidy-issues |
| [#422](https://github.com/missingbulb/Shepherd/issues/422) |  | unlabelled-backlog |  | 2026-09-02 | Dashboard Pages deploy: move the trigger, build and follow-through into a publish-pages task |
...
| [#127](https://github.com/missingbulb/Shepherd/issues/127) |  | schedule-board |  | 2026-09-03 | [claudinite-schedule] the schedule board |
...
## missingbulb/ClaudiniteCanary — 17 open (10 queue / 7 plain)

| # | Q | State | Gen | Updated | Title |
|---|---|-------|-----|---------|-------|
| [#331](https://github.com/missingbulb/ClaudiniteCanary/issues/331) | Q | running-executor | canon | 2026-09-03 | [claudinite-work] claudinite-lifecycle/update |
...
| [#148](https://github.com/missingbulb/ClaudiniteCanary/issues/148) |  | schedule-board |  | 2026-09-03 | [claudinite-schedule] the schedule board |
...
## missingbulb/VascularColoring — 14 open (9 queue / 5 plain)

| # | Q | State | Gen | Updated | Title |
|---|---|-------|-----|---------|-------|
| [#336](https://github.com/missingbulb/VascularColoring/issues/336) | Q | running-executor | canon | 2026-09-03 | [claudinite-work] claudinite-lifecycle/update |
...
| [#171](https://github.com/missingbulb/VascularColoring/issues/171) |  | schedule-board |  | 2026-09-03 | [claudinite-schedule] the schedule board |
